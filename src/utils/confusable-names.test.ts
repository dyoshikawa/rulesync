...
describe("describeConfusableNames", () => {
  it("should note nothing for names that cannot be mistaken for each other", () => {
    expect(describeConfusableNames({ names: ["skill-a", "skill-b"] })).toEqual(new Map());
  });

  it("should note both entries that share a display form", () => {
    const notes = describeConfusableNames({ names: ["Skill", "skill"] });
  ...
  it("should fold compatibility forms when comparing display forms", () => {
    ...
    const fullwidth = "\uff50\uff44\uff46";
    const notes = describeConfusableNames({ names: ["pdf", fullwidth] });
  ...
  it("should fold invisible characters when comparing display forms", () => {
    ...
    const notes = describeConfusableNames({ names: ["pdf", invisible] });
  ...
  it("should note a name spelled entirely in another script beside its twin", () => {
    ...
    const notes = describeConfusableNames({ names: ["copy", CYRILLIC_COPY] });
  ...
  it("should note a name written entirely in Latin lookalikes with nothing to compare it to", () => {
    ...
    const notes = describeConfusableNames({ names: [CYRILLIC_COPY, "unrelated"] });
  ...
  it("should not report an ordinary word of a script that merely has some lookalikes", () => {
    ...
    expect(
      describeConfusableNames({ names: ["\u043f\u0440\u0430\u0432\u0438\u043b\u0430"] }),
    ).toEqual(new Map());
  ...
  it("should not call two same-length names twins when no letter shape matches", () => {
    ...
    expect(describeConfusableNames({ names: ["rules", "\u03bb\u03cc\u03b3\u03bf\u03c2"] })).toEqual(
      new Map(),
    );
  });
  ...
  it("should fold whitespace differences into the display form", () => {
    ...
    const padded = "pdf  ";
    const notes = describeConfusableNames({ names: ["pdf", padded] });
  ...
  it.each([
    ...
    // be seen of it.
    expect(describeConfusableNames({ names: [name] })).toEqual(new Map([[name, WHITESPACE_NOTE]]));
  });
  ...
  it.each([
    ...
    expect(describeConfusableNames({ names: [name] })).toEqual(new Map());
  });
  ...
  it("should still pair a name that swaps a blank for one drawn like it", () => {
    ...
    const notes = describeConfusableNames({ names: ["pdf reader", wide] });
  ...
  it("should not note names that differ within a single script", () => {
    expect(describeConfusableNames({ names: ["cat", "dog", "cap"] })).toEqual(new Map());
  });

  it("should note a mixed-script name even when no lookalike is on the list", () => {
    const notes = describeConfusableNames({ names: [HALF_CYRILLIC_GOOD] });
  ...
  it("should note every reason that applies to the same name", () => {
    const notes = describeConfusableNames({ names: ["good", HALF_CYRILLIC_GOOD] });
  ...
  it("should list three scripts as a sentence rather than a chain of ands", () => {
    ...
    const notes = describeConfusableNames({ names: [threeScripts] });
  ...
  it("should not name the internal bucket an unlisted script falls into", () => {
    ...
    const withArabic = "\u0440\u0645";
    const notes = describeConfusableNames({ names: [withArabic] });
  ...
  it("should fold an invisible character that normalization turns into another", () => {
    ...
    const notes = describeConfusableNames({ names: ["pdf", hangulFiller] });
  ...
  it("should not call two names of the same length twins when nothing links them", () => {
    ...
    expect(describeConfusableNames({ names: ["\u8a2d\u5b9a", "ai"] })).toEqual(new Map());
    expect(describeConfusableNames({ names: ["\uaddc\uce59", "go"] })).toEqual(new Map());
  });
  ...
  it("should note a twin written in an alphabet only UTS #39 would name", () => {
    ...
    const armenian = "c\u0585py";
    const notes = describeConfusableNames({ names: ["copy", armenian] });
  ...
  it("should note a twin that never leaves the Latin alphabet", () => {
    ...
    expect(describeConfusableNames({ names: ["copy", "c0py"] })).toEqual(
      new Map([
        ["copy", TWIN_NOTE],
        ["c0py", TWIN_NOTE],
      ]),
    );
    expect(describeConfusableNames({ names: ["rules", "ruIes"] })).toEqual(
      new Map([
        ["rules", TWIN_NOTE],
        ["ruIes", TWIN_NOTE],
      ]),
    );
    expect(describeConfusableNames({ names: ["git", "\u0261it"] })).toEqual(
      new Map([
        ["git", TWIN_NOTE],
        ["\u0261it", TWIN_NOTE],
      ]),
    );
    expect(describeConfusableNames({ names: ["git", "g\u0131t"] })).toEqual(
      new Map([
        ["git", TWIN_NOTE],
        ["g\u0131t", TWIN_NOTE],
      ]),
  ...
  it("should note a twin the compatibility normalization would have hidden", () => {
    ...
    expect(describeConfusableNames({ names: ["list", "\u2160ist"] })).toEqual(
      new Map([
        ["list", TWIN_NOTE],
        ["\u2160ist", TWIN_NOTE],
      ]),
  ...
  it("should leave an ordinary word in a lookalike alphabet alone", () => {
    ...
    expect(describeConfusableNames({ names: ordinary })).toEqual(new Map());
  });

  it("should still report a word whose every letter is drawn as a Latin one", () => {
    expect(describeConfusableNames({ names: [CYRILLIC_COPY] }).get(CYRILLIC_COPY)).toBe(
  ...
  it("should report an alphabet drawn like Latin that the ordinary mixtures once hid", () => {
    ...
    expect(describeConfusableNames({ names: [osage] }).get(osage)).toBe(
  ...
  it("should name the third entry that reads like two others", () => {
    ...
    const notes = describeConfusableNames({ names: ["copy", "copy ", "c\u043epy"] });
  ...
  it("should note a twin that only replaces the hyphen", () => {
    ...
    expect(describeConfusableNames({ names: ["code-review", "code\u2010review"] })).toEqual(
      new Map([
  ...
  it("should note a twin that only replaces the apostrophe", () => {
    expect(describeConfusableNames({ names: ["let's-go", "let\u2019s-go"] })).toEqual(
      new Map([
        ["let's-go", TWIN_NOTE],
        ["let\u2019s-go", TWIN_NOTE],
      ]),
  ...
  it("should report a name written wholly in an alphabet drawn like Latin", () => {
    ...
    expect(
      describeConfusableNames({ names: ["\ua4d1\ua4d3\ua4de"] }).get("\ua4d1\ua4d3\ua4de"),
  ...
  it.each([
    ...
    expect(describeConfusableNames({ names: [name] }).get(name)).toBe(
  ...
  it("should not report a repeated name as colliding with itself", () => {
    expect(describeConfusableNames({ names: ["skill-a", "skill-a"] })).toEqual(new Map());
  });
