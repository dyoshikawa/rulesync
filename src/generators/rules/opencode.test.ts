...
describe("generateOpenCodeConfig", () => {
  ...
  it("should generate .opcodeignore when .rulesyncignore exists", async () => {
    ...
    expect(results).toHaveLength(3); // 1 memory + 1 root + 1 .opcodeignore

    const opcodeIgnoreFile = results.find((r) => r.filepath === ".opcodeignore");
    expect(opcodeIgnoreFile).toBeDefined();
    expect(opcodeIgnoreFile?.content).toContain("# Generated by rulesync from .rulesyncignore");
    expect(opcodeIgnoreFile?.content).toContain("*.test.md");
    expect(opcodeIgnoreFile?.content).toContain("temp/**/*");
  });
  ...
  it("should respect baseDir parameter for .opcodeignore", async () => {
    vi.mocked(loadIgnorePatterns).mockResolvedValue({
      patterns: ["*.test.md"],
    });
    ...
    const opcodeIgnoreFile = results.find((r) => r.filepath.endsWith(".opcodeignore"));
    expect(opcodeIgnoreFile?.filepath).toBe("/custom/base/.opcodeignore");
  });
