...
describe("CopilotRule", () => {
  ...
  describe("toRulesyncRule", () => {
    ...
    it("should convert root CopilotRule to RulesyncRule", () => {
      ...
      // applyTo is preserved verbatim; root rule's globs come solely from applyTo.
      expect(rulesyncRule.getFrontmatter()).toEqual({
        targets: ["*"],
        root: true,
        description: "Root rule",
    ...
    it("should not inject globs when root CopilotRule has no applyTo", () => {
      // Root files generated by rulesync itself contain no frontmatter at all.
    ...
    it("should preserve 'no globs' across a generate/import cycle for root rules", () => {
