...
describe("ZcodeRule", () => {
  ...
  describe("getSettablePaths", () => {
    it("should return the project-root AGENTS.md for project scope", () => {
      const paths = ZcodeRule.getSettablePaths();
    ...
    it("should return the profile-directory AGENTS.md for global scope", () => {
      const paths = ZcodeRule.getSettablePaths({ global: true });
      expect(paths.root.relativeDirPath).toBe(".zcode");
      expect(paths.root.relativeFilePath).toBe("AGENTS.md");
    });
  });
  ...
  describe("fromFile", () => {
    it("should load the root rule from AGENTS.md", async () => {
      const content = "# ZCode Instructions\n\nUse TypeScript.";
      await writeFileContent(join(testDir, "AGENTS.md"), content);

      const rule = await ZcodeRule.fromFile({
        outputRoot: testDir,
    ...
    it("should read the global AGENTS.md from .zcode/", async () => {
      const content = "# Global ZCode Instructions";
      await writeFileContent(join(testDir, ".zcode", "AGENTS.md"), content);
      ...
      expect(rule.getFileContent()).toBe(content);
      expect(rule.getRelativeDirPath()).toBe(".zcode");
    });
  });
  ...
  describe("fromRulesyncRule", () => {
    it("should write a root rule to the project-root AGENTS.md", () => {
      ...
      const rule = ZcodeRule.fromRulesyncRule({ outputRoot: testDir, rulesyncRule });
    ...
    it("should write to .zcode/AGENTS.md in global mode", () => {
      const rulesyncRule = new RulesyncRule({
        outputRoot: testDir,
        relativeDirPath: "rules",
        relativeFilePath: "root.md",
      ...
      const rule = ZcodeRule.fromRulesyncRule({ outputRoot: testDir, rulesyncRule, global: true });
  ...
  describe("forDeletion", () => {
    it("should mark the project-root AGENTS.md as root", () => {
      const rule = ZcodeRule.forDeletion({
        outputRoot: testDir,
        relativeDirPath: ".",
        relativeFilePath: "AGENTS.md",
      });
    ...
    it("should mark the global .zcode/AGENTS.md as root", () => {
      const rule = ZcodeRule.forDeletion({
        outputRoot: testDir,
  ...
  describe("isTargetedByRulesyncRule", () => {
    ...
    it("should return true for rules targeting zcode", () => {
      expect(ZcodeRule.isTargetedByRulesyncRule(buildRule(["zcode"]))).toBe(true);
    });

    it("should return true for rules targeting all tools (*)", () => {
      expect(ZcodeRule.isTargetedByRulesyncRule(buildRule(["*"]))).toBe(true);
    });

    it("should return false for rules not targeting zcode", () => {
      expect(ZcodeRule.isTargetedByRulesyncRule(buildRule(["cursor", "copilot"]))).toBe(false);
    });
  });
});
