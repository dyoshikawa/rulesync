...
export type AntigravityIdeCommandParams = {
 ...
 *
 * Generates workflow files for Antigravity's `.agents/workflows/` directory
 * (project scope) and `~/.gemini/antigravity/global_workflows/` (global scope).
 * The workflow body/frontmatter shape matches {@link AntigravityCommand}.
 */
export class AntigravityIdeCommand extends ToolCommand {
  ...
  static getSettablePaths({ global = false }: { global?: boolean } = {}): ToolCommandSettablePaths {
    if (global) {
      return {
        relativeDirPath: join(".gemini", "antigravity", "global_workflows"),
      };
    }
    return {
  ...
  constructor({ frontmatter, body, ...rest }: AntigravityIdeCommandParams) {
    if (rest.validate) {
  ...
  static fromRulesyncCommand({
    ...
    validate = true,
    global = false,
  }: ToolCommandFromRulesyncCommandParams): AntigravityIdeCommand {
    const rulesyncFrontmatter = rulesyncCommand.getFrontmatter();
  ...
  static async fromFile({
    ...
    validate = true,
    global = false,
  }: ToolCommandFromFileParams): Promise<AntigravityIdeCommand> {
    const paths = this.getSettablePaths({ global });
  ...
  static forDeletion({
    ...
    relativeDirPath,
    relativeFilePath,
  }: ToolCommandForDeletionParams): AntigravityIdeCommand {
    return new AntigravityIdeCommand({
      outputRoot,
      relativeDirPath,
      relativeFilePath,
