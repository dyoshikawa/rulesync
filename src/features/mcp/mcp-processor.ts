  ...
  async convertRulesyncFilesToToolFiles(rulesyncFiles: RulesyncFile[]): Promise<ToolFile[]> {
    ...
    const toolMcps = await Promise.all(
      [rulesyncMcp].map(async (mcp) => {
        ...
        // deprecated per-server `targets` filter) for this target first.
        const targetMcp = mcp.forTarget({ toolTarget: this.toolTarget, logger: this.logger });
        ...
        const filteredRulesyncMcp = targetMcp.stripMcpServerFields(fieldsToStrip);
