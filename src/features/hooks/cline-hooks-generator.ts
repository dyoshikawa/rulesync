...
export function generateClineHookScript({
  ...
  const lines = [
    ...
    "",
    // The mirror of the `.ps1` guard, for the one quadrant where both spellings
    // really do execute the commands: Windows with a POSIX shell on PATH. The
    // SDK/CLI infers this file's interpreter from its shebang and normalizes it
    // to a bare `bash` with no platform check, so under Git Bash it runs here
    // while the twin runs under PowerShell. On Windows the `.ps1` is the one
    // that works (its commands go through `cmd /c`), so this side stands down.
    // The VS Code extension never spawns this file on Windows, and no Unix
    // shell reports these `$OSTYPE` values, so nothing else is affected.
