...
body`,
  ...
  describe("fromRulesyncSkill", () => {
    ...
    it("should fall back to the root-level license/compatibility/metadata when the rovodev section omits them", () => {
      const rulesyncSkill = new RulesyncSkill({
        outputRoot: testDir,
        relativeDirPath: RULESYNC_SKILLS_RELATIVE_DIR_PATH,
        dirName: "root-fields",
      ...
      const frontmatter = RovodevSkill.fromRulesyncSkill({ rulesyncSkill }).getFrontmatter();
      expect(frontmatter.license).toBe("MIT");
      expect(frontmatter.compatibility).toBe("Requires git");
      expect(frontmatter.metadata).toEqual({ author: "root" });
    });

    it("should let the rovodev section override the root-level license/compatibility/metadata", () => {
      const rulesyncSkill = new RulesyncSkill({
        outputRoot: testDir,
        relativeDirPath: RULESYNC_SKILLS_RELATIVE_DIR_PATH,
