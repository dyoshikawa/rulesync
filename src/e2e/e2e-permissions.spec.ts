...
`,
  ...
  it("should generate zoocode permissions into .vscode/settings.json", async () => {
      ...
      JSON.stringify(
        {
          permission: {
            bash: { "git ": "allow", "rm -rf": "deny", "npm ": "ask" },
            // Zoo Code gates terminal commands only; other categories have no
    ...
    await runGenerate({ target: "zoocode", features: "permissions" });
...
`,
  ...
  it("should import zoocode permissions into .rulesync/permissions.jsonc", async () => {
    const testDir = getTestDir();
    ...
    await writeFileContent(
      ...
      JSON.stringify(
        {
          "editor.tabSize": 2,
          "zoo-code.allowedCommands": ["git "],
          "zoo-code.deniedCommands": ["rm -rf"],
        },
        null,
        2,
      ),
    ...
    await runImport({ target: "zoocode", features: "permissions" });
