...
dist/`;
    ...
    it("should wrap the managed block with start and end markers", async () => {
      vi.mocked(fileExists).mockResolvedValue(false);
      ...
      const startIndex = content.indexOf("# Generated by Rulesync");
      const endIndex = content.indexOf("# End of Rulesync");
      expect(startIndex).toBeGreaterThanOrEqual(0);
      expect(endIndex).toBeGreaterThan(startIndex);
    });

    it("should remove a marker-delimited block deterministically, even with unknown lines inside", async () => {
      // The end marker lets us drop the whole block regardless of its content,
      // including a stray line that is not a known rulesync entry.
      const existingContent = `node_modules/

# Generated by Rulesync
**/.cursor/rules/
**/.stale-entry-from-old-version/
# End of Rulesync
...
dist/`;
      ...
      // The stale entry inside the old block must not survive.
      expect(content).not.toContain("**/.stale-entry-from-old-version/");
      // Exactly one managed block remains (single start and end marker).
      expect(content.match(/# Generated by Rulesync/g)).toHaveLength(1);
      expect(content.match(/# End of Rulesync/g)).toHaveLength(1);
  ...
  describe("when previously managed entries are no longer emitted", () => {
    it("should warn about entries removed from the managed block", async () => {
      const entries = filterGitignoreEntries();
      const staleBlock = `# Generated by Rulesync\n**/.obsolete/config.toml\n${entries.join(
        "\n",
      )}\n# End of Rulesync\n`;

      vi.mocked(fileExists).mockImplementation(async (path: string) => path.endsWith(".gitignore"));
      vi.mocked(readFileContent).mockImplementation(async (path: string) =>
        path.endsWith(".gitignore") ? `node_modules/\n\n${staleBlock}` : "",
      );

      await gitignoreCommand(mockLogger);

      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("no longer gitignored"));
      expect(mockLogger.warn).toHaveBeenCalledWith("  **/.obsolete/config.toml");
    ...
    it("should leave the entry list to captured data under --json", async () => {
      const entries = filterGitignoreEntries();
      const staleBlock = `# Generated by Rulesync\n**/.obsolete/config.toml\n${entries.join(
        "\n",
      )}\n# End of Rulesync\n`;
      ...
      const jsonLogger = { ...mockLogger, jsonMode: true };

      await gitignoreCommand(jsonLogger);

      // The warnings array a `--json` run reports is bounded, and the entries
      // are already in `data.entriesRemoved`; the sentences that are not go on.
      expect(jsonLogger.captureData).toHaveBeenCalledWith("entriesRemoved", [
        "**/.obsolete/config.toml",
      ]);
    ...
    it("should count more than one entry in the plural under --json", async () => {
      const entries = filterGitignoreEntries();
      const staleBlock = `# Generated by Rulesync\n**/.obsolete/config.toml\n**/.retired/settings.json\n${entries.join(
        "\n",
      )}\n# End of Rulesync\n`;
      ...
      expect(jsonLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('2 entries listed under "entriesRemoved" were removed'),
      );
      expect(jsonLogger.warn).toHaveBeenCalledWith(expect.stringContaining("are no longer"));
    });
