...
export async function installCommand(options: InstallCommandOptions): Promise<void> {
  ...
  if (result.fetchedItemCount > 0) {
    logger.success(
      `Installed ${result.fetchedItemCount} item(s) from ${result.sourcesProcessed} source(s).`,
    );
  } else {
    logger.success(`All items up to date (${result.sourcesProcessed} source(s) checked).`);
  }
}
