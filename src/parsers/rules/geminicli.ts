...
/**
 * Parser for Gemini CLI rule files
 */
export class GeminiCLIRuleParser extends BaseRuleParser {
  getToolName() {
    return "geminicli" as const;
  }

  getRuleFilesPattern(): string[] {
    return ["GEMINI.md", ".gemini/memories/*.md"];
  }
  ...
  private async parseMainFile(baseDir: string, result: RuleParseResult): Promise<void> {
    const mainFilePath = resolvePath("GEMINI.md", baseDir);
    if (!(await fileExists(mainFilePath))) {
      result.errors.push("GEMINI.md file not found");
      return;
    }
  ...
  private async parseMemoryFiles(baseDir: string, result: RuleParseResult): Promise<void> {
    const memoryDir = resolvePath(".gemini/memories", baseDir);
