...
Codex-native profile fields — `extends`, `description`, `network.enabled`, `network.mode`, and `network.unix_sockets` — have no equivalent in Rulesync's canonical permissions model and are not generated by Rulesync. However, if an existing `.codex/config.toml` already contains these fields on the `rulesync` profile, Rulesync preserves them on every regeneration so they are not lost on round-trip. Note that `filesystem` and `network.domains` are always overwritten by Rulesync (they are the canonical source), so any hand-authored values in those sections will be replaced on regeneration.
