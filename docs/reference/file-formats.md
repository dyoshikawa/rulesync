...
The `:workspace_roots` table also always receives two default `.git` carve-outs: `".git/**" = "write"` and `".git/config" = "read"`. Codex's `:workspace` baseline keeps `.git` read-only inside workspace roots, which denies basic git workflows (commit/stage operations write to `.git/index`, `.git/objects`, refs, and logs). The `".git/**"` write rule reopens the subtree, while `".git/config"` stays read-only as a security guard — a writable `.git/config` would let a sandboxed process set keys like `core.fsmonitor` or `core.hooksPath` that execute code outside the sandbox (Codex resolves the more specific path with priority, so the `read` rule wins for that file). A user rule for the same pattern always wins over the default, and setting the `codexcli.git_write_rules` override to `false` suppresses the carve-outs entirely (only an explicit `false` does; the default is `true`). Like `:minimal`, the default-valued carve-outs are not imported into the Rulesync model on `rulesync import` — they are re-added on every generate — while customized `.git` values import normally.
