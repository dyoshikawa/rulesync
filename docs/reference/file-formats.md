...
> **Supporting-file note:** every file beside `SKILL.md` in a skill directory is copied **byte for byte**, to whichever tool root the skill is generated into. Most of them are user assets — images, archives, fixtures whose CRLF line endings or missing trailing newline are deliberate — so unlike `SKILL.md`, whose body and frontmatter Rulesync composes, they get no UTF-8 round-trip, no line-ending normalization and no trailing newline appended. Change detection starts from byte equality and, for a supporting file with a structured extension (`.json`, `.yaml`, `.toml`, `.md`), falls back to comparing the parsed content, so reformatting a file Rulesync generates itself (Codex CLI's `agents/openai.yaml`) does not report a change on every generate. A supporting file written by an older Rulesync, which normalized text files, is rewritten once on the next generate — or left alone if its extension makes it structurally comparable.
