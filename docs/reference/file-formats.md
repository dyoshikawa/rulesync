...
Generating checks for Cursor overwrites `.cursor/BUGBOT.md`, so run `rulesync import --targets cursor --features checks` first if the repository already has a hand-written one. Deletion is guarded: a `BUGBOT.md` carrying no rulesync marker is never removed — dropping the last check that targets Cursor takes rulesync's own output with it, not somebody's hand-written review instructions.
