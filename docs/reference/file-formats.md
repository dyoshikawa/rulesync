...
- `webfetch`: `allow`/`deny` map to `permissions.<profile>.network.domains` (Codex does not support `ask` for domain rules); `network.enabled = true` is emitted only when at least one `allow` rule is present. Deny-only domain sets are emitted without `enabled`, which Codex treats as restricted (its default) while the deny entries still round-trip back into Rulesync rules. Codex rejects the global wildcard `*` in `network.domains` at config load time, so `webfetch: { "*": "allow" }` is emitted as `network.enabled = true` with no domain entry, and `webfetch: { "*": "deny" }` is skipped with a warning (unlisted domains are denied by Codex's allowlist-first policy anyway). A Codex profile with `network.enabled = true` but no `domains` is imported as `webfetch: { "*": "allow" }`, so this shape round-trips losslessly.
