...
- `enabled` (optional): boolean, default `true`. Whether the hook is active. Forwarded to Kiro IDE (`.kiro/hooks/rulesync.json`), the only tool with an on-disk enable flag; an imported `enabled: false` round-trips, so a deliberately disabled hook is not silently switched back on by the next generate.
