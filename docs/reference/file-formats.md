...
Multiple files can set `root: true` for the same target in project and global modes. Rulesync renders each file through the target adapter, then combines compatible root or single-file outputs in deterministic source-discovery order with one blank line between fragments. Targets that map source rules to distinct native paths keep those files separate. If modular rules normalize to the same output path, or if generated paths differ only by case, generation fails instead of emitting malformed or platform-dependent files.
