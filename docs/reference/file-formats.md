...
Multiple files can set `root: true` for the same target in project and global modes. Rulesync renders each file through the target adapter, then combines compatible root or single-file outputs in deterministic source-discovery order with one blank line between fragments. Fragments are ordered lexicographically by source file path, so filename prefixes such as `10-` and `20-` control composition order. Targets that map source rules to distinct native paths keep those files separate. If modular rules normalize to the same output path, generation fails instead of emitting malformed files or silently overwriting content.
