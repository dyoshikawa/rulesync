...
Multiple files can set `root: true` for the same target in project and global modes. Rulesync renders each file through the target adapter, then combines files that resolve to the same output path in deterministic source-discovery order with one blank line between fragments. Targets that map source rules to distinct native paths keep those files separate.
