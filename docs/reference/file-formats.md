...
> **Kimi Code note:** Kimi Code writes the root rule to the auto-loaded `AGENTS.md` — project `.kimi-code/AGENTS.md` (with the project-root `./AGENTS.md` accepted on import) and global `~/.agents/AGENTS.md` (via `--global`, the AGENTS open standard) — as plain Markdown. Kimi Code has no non-root instruction directory, so Rulesync folds every non-root rule body into the single root `AGENTS.md`.
...
> **Kimi Code note:** MCP servers are written verbatim to the `mcpServers` map of a dedicated `.kimi-code/mcp.json` — the same relative path for project and global scope (global resolves under `~/.kimi-code/`, via `--global`). There is no per-server tool allow/deny list. Because `mcp.json` is a dedicated file (not a shared settings file), Rulesync manages it as normal output. Other top-level keys in `mcp.json` are preserved on round-trip.
