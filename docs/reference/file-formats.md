...
For Goose, this generates the `user` block of the global `~/.config/goose/permission.yaml` (**global mode only** — Goose persists per-tool permission overrides only under the home directory and has no project-scoped permissions file). Goose stores permissions as a YAML map of mode key → `{ always_allow, ask_before, never_allow }`, where each field is a list of tool-name strings; rulesync writes the user-set decisions under the `user` key. Action mapping is a 1:1: `allow` → `always_allow`, `ask` → `ask_before`, `deny` → `never_allow`. Tool-name mapping: `bash` → `developer__shell`, `edit` → `developer__text_editor`; every other category passes through verbatim as the Goose tool name (so namespaced tools like `developer__text_editor` or `developer__image_processor` round-trip). Because Goose permission lists hold **whole tool names** rather than per-command/per-path globs, only a category's catch-all `*` pattern is representable — non-catch-all patterns are skipped with a warning. `write` collapses onto `developer__text_editor` too, so a conflicting `edit`/`write` catch-all cannot be represented; `edit` takes precedence and a warning is logged. The `permission.yaml` file is merged in place: the `user` block's managed disposition lists are owned by rulesync, while other sub-keys of `user` and every other top-level key are preserved, and the file is never deleted. See the [Goose tool permissions docs](https://block.github.io/goose/docs/guides/tool-permissions/).

> **Goose-only override (`goose` key):** Goose has a fourth per-tool disposition, `smart_approve` (defer the decision to Goose's LLM PermissionJudge), that is semantically distinct from the canonical `ask` (which prompts the human) and cannot be expressed by `allow`/`ask`/`deny`. Add a tool-scoped `goose` override to author it: entries under `goose.permission` map a canonical category to `"smart_approve"` and are emitted into the `user.smart_approve` list of `permission.yaml`; the shared `permission` block continues to drive `always_allow`/`ask_before`/`never_allow`. On import, the `user.smart_approve` list round-trips back into the `goose` override. (The global `goose_mode` setting lives in Goose's separate `config.yaml`, not `permission.yaml`, so it is out of scope here; the `GOOSE_ALLOWLIST` MCP-extension governance is likewise not a permission surface.)
>
> ```json
> {
>   "permission": { "bash": { "*": "allow" } },
>   "goose": { "permission": { "edit": "smart_approve" } }
> }
> ```

For the Grok Build CLI (`grokcli`), this generates the `[ui] permission_mode` toggle in the global `~/.grok/config.toml` (**global mode only** — Grok has no project-scoped permissions file). Grok does not have per-tool / per-pattern rules; tool gating is a single coarse toggle, `permission_mode = "ask"` (prompt on each tool call; Grok's default) or `"always-approve"` (skip prompts). The mapping is therefore **lossy**: on generate, any `deny`/`ask` rule anywhere collapses to `ask` (conservative — keep prompting whenever the user expressed any restriction), otherwise an existing `allow` rule with no denies/asks opts into `always-approve`, and an empty config defaults to `ask`. On import, `always-approve` ⇄ `bash: { "*": "allow" }` and `ask` (or unset) ⇄ `bash: { "*": "ask" }` (`bash` is the representative catch-all). `config.toml` is shared with the MCP feature, so the `[ui] permission_mode` value is merged in place — every other key (e.g. `[mcp_servers]`, the legacy `approval_mode`) is preserved — and the file is never deleted. See the [Grok CLI modes docs](https://docs.x.ai/build/modes-and-commands).
