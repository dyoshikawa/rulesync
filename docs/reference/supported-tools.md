...
- **Continue (`continue`)** — Continue is end of life: Continue [joined Cursor](https://continue.dev) on 2026-06-18, the [continuedev/continue repository](https://github.com/continuedev/continue) is read-only with a final **v2.0.0** VS Code release (2026-06-19) and its last commit on 2026-07-21, and the in-product deprecation banner expired on 2026-09-09. The `continue` target stays supported as a frozen-compatibility target: every adapter was verified against that final source tree, and nothing about the file formats will move again, so existing output keeps working for anyone still running the last release. Do not expect new Continue capabilities to be tracked; new projects should pick an actively maintained tool instead.
...
- **Tabnine CLI (`tabnine`)** — The file-based Tabnine CLI this target models is in [maintenance mode](https://docs.tabnine.com/main/getting-started/tabnine-cli): as of Tabnine **6.6** (2026-09-15) it is ["Tabnine CLI (Legacy)"](https://docs.tabnine.com/main/administering-tabnine/release-notes), installed as `tabnine-legacy` once the new CLI is present, and it receives only critical updates through **2026-12-31**, after which Tabnine deprecates it fully. The current `tabnine` command is [Tabnine's distribution of OpenCode](https://docs.tabnine.com/main/getting-started/tabnine-plugin-for-opencode), configured with standard OpenCode files — so for the new CLI use **`--targets opencode`** (`AGENTS.md`, `opencode.json`, `.opencode/{commands,agents,skills}`, `~/.config/opencode/**`); its [migration helper](https://docs.tabnine.com/main/getting-started/tabnine-plugin-for-opencode/migrating-from-tabnine-cli) (`/migrate`, `/migrate-context`) carries MCP servers, skills, agents, commands and `TABNINE.md` over. The `tabnine` target stays supported for the legacy CLI (0.35.0) until then; whether it is removed in a later major is decided separately.
