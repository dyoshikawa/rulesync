...
The first two reasons are asked of the skills already in the output directory as well as of the other entries, and say `a local skill` in place of `another entry` when that is where the twin is. Without that comparison a repository publishing only the imitation — `dep1oy` against a `deploy` you have had for months — is a single plain-ASCII name in one script with nothing on the list to compare it against, and every check stays quiet. A local skill that names the same directory as an entry is the skill that entry would refresh rather than one imitating it, so it is not a collision and is not marked; a second fetch of the same repository is therefore as quiet as the first. Same directory covers more than an identical spelling: a local `pdf` beside an entry named `PDF`, and a name stored decomposed beside the composed spelling of it, are the pairs macOS and Windows resolve to one directory, so they are taken as a refresh too — otherwise every accented or Japanese skill name on such a volume would be marked on every fetch. Where they really are two directories, the prune step says so after the write. A fullwidth `ｐｄｆ` is a second directory everywhere and stays marked. Where both another entry and a local skill collide with a name, the entry is the one named, since that is the pair you can compare on screen.
