...
- **Another entry differs from it only by lookalike letters.** Two names that read the same once each character is replaced by the Latin letter it is drawn as — `copy` beside the same word spelled entirely in Cyrillic. A name does not have to leave the Latin alphabet to qualify: `c0py` with a zero, `ruIes` with a capital I for the l, and `git` with the script `ɡ` or the dotless `ı` are all marked against the plain spelling. Neither name mixes scripts on its own, so this pair is visible only by comparing the two.
