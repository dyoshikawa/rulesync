...
describe("getToonFiles", () => {
  it("should return only .toon files", async () => {
  ...
  it("should return empty array when no .toon files exist", async () => {
    const { testDir, cleanup } = await setupTestDirectory();
    try {
...
describe("loadPrompt", () => {
  it("should read prompt from file", async () => {
    ...
    try {
      ...
      writeFileSync(promptPath, "Analyze this code for vulnerabilities.");

      const prompt = loadPrompt({ promptPath });
      expect(prompt).toBe("Analyze this code for vulnerabilities.");
    } finally {
      await cleanup();
    }
  });
  ...
  it("should throw when file does not exist", () => {
    expect(() => loadPrompt({ promptPath: "/nonexistent/prompt.txt" })).toThrow();
  });
});
...
describe("formatEmailBody", () => {
  ...
  it("should format multiple files", () => {
    ...
    expect(body).toContain("Found 1 vulnerabilities");
  });
});
