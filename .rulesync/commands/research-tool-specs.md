...
$target_tool_name, $tool_name_in_rulesync, $overwrite, $instructions = $ARGUMENTS
...
First, check if the following files already exist.
If $overwrite is true, even though the files alrepdy exists, please overwrite them. If $overwrite is false, please skip the research and creation of the files that already exist.
If $instructions is provided, please consider its content when performing the following tasks.
