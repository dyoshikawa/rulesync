...
- The distributed skill `skills/rulesync/` ships only `SKILL.md`, which delegates detailed lookup to the `rulesync docs` CLI command. `scripts/sync-skill-docs.ts` and `scripts/check-skill-docs-sync.ts` enforce that rule for `skills/rulesync/` alone, so other distributed skills (e.g. `skills/rulesync-mcp/`) may ship `references/` and other supporting files. The bundled documentation itself is embedded from `docs/**/*.md` into `src/generated/docs-content.ts` by `scripts/generate-docs-content.ts` (regenerate with `pnpm run generate:docs-content`; CI fails on drift).
