...
- When you change how the MCP server behaves (`src/mcp/**`), update both `docs/reference/mcp-server.md` and the distributed skill `skills/rulesync-mcp/SKILL.md`. The skill restates the same facts so that it reads standalone, and nothing checks the two for drift automatically.
