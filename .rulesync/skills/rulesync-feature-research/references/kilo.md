...
| `hooks`       | `https://kilo.ai/docs/automate/extending/plugins`                      | Plugin modules auto-registered from `.kilo/plugins` (also the singular `plugin/`, legacy `.kilocode/plugin/`) and `~/.config/kilo/plugin`; lifecycle/tool/chat hooks plus a catch-all `event` hook          |
