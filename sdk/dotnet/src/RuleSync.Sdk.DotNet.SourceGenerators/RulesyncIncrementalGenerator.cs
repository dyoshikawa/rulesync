...
public class RulesyncIncrementalGenerator : IIncrementalGenerator
{
        ...
        string sourcePath)
    {
        ...
        foreach (var recordDef in records.Values)
        {
            ...
            if (addedHintNames.Add(hintName))
            {
                var source = GenerateRecordSource(recordDef);
                context.AddSource(hintName, source);
            }
        }
    ...
    private static string GenerateRecordSource(RecordDefinition recordDef)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("#nullable enable");
        ...
        IReadOnlyList<RecordDefinition> records)
    {
        ...
        sb.AppendLine("using System.Text.Json.Serialization;");
        sb.AppendLine();
        sb.AppendLine("namespace Rulesync.Sdk.Serialization;");
        sb.AppendLine();
        sb.AppendLine("[JsonSourceGenerationOptions(");
        sb.AppendLine("    PropertyNamingPolicy = JsonKnownNamingPolicy.CamelCase,");
        sb.AppendLine("    DefaultBufferSize = 4096)]");

        foreach (var enumDef in enums)
        {
            sb.AppendLine($"[JsonSerializable(typeof(Models.{enumDef.Name}))]");
        }

        foreach (var recordDef in records)
        {
            sb.AppendLine($"[JsonSerializable(typeof(Models.{recordDef.Name}))]");
        }

        sb.AppendLine("internal partial class RulesyncJsonContext : JsonSerializerContext");
        sb.AppendLine("{");
        sb.AppendLine("}");
