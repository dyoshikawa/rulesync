...
Rulesync follows symbolic links during file discovery across **all** features (rules, commands, subagents, skills, MCP, ignore, permissions), whether you use a plain `.rulesync/` directory or a separate `--input-root`. A symlink inside the input tree that points elsewhere is followed transparently, and the resolved file content is copied into the generated output. This is intentional: it lets you centralize shared skills or rules in one place and reference them via symlinks without duplication (see [issue #1707](https://github.com/dyoshikawa/rulesync/issues/1707)).
