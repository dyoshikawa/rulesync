...
- Targets that write the same output file resolve to a shared key: `kiro-cli` / `kiro-ide` fall back to the `kiro` block and `claudecode-legacy` to the `claudecode` block (an exact `{toolname}` key still wins). `antigravity-cli` and `antigravity-ide` have separate keys but share the project-scope `.agents/mcp_config.json`, so keep their blocks consistent when generating both.
- The per-server `targets` array (e.g. `"targets": ["claudecode"]`) is **deprecated**: it is still honored (a server whose `targets` names neither `"*"` nor the tool is excluded from that tool's output, with warnings naming the excluded servers), but new configurations should express tool-specific servers through the `{toolname}.mcpServers` blocks above. Note that earlier Rulesync versions parsed `targets` without applying it, so upgrading enforces filters that previously had no effect — review any leftover `targets` entries.
