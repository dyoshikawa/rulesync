...
The `:workspace_roots` table also receives two default `.git` carve-outs: `".git/**" = "write"` and `".git/config" = "read"`. Codex's `:workspace` baseline keeps `.git` read-only inside workspace roots, which denies basic git workflows (commit/stage operations write to `.git/index`, `.git/objects`, refs, and logs). The `".git/**"` write rule reopens the subtree, while `".git/config"` stays read-only — a writable `.git/config` would let a sandboxed process set keys like `core.fsmonitor` or `core.hooksPath` that execute code outside the sandbox (Codex resolves the more specific path with priority, so the `read` rule wins for that file). Note this guard is deliberately partial: `.git/hooks/` (and `.git/modules/**` for submodules) remains writable so hook managers such as lefthook and simple-git-hooks keep working; a sandboxed process could still install a hook directly, and users who want stricter isolation can author a more specific rule (e.g. `read: { ".git/hooks/**": "allow" }`) in the canonical permissions, which wins over the default. Because `.git/**` is an unbounded `**` pattern, the carve-outs also mean `glob_scan_max_depth = 8` is effectively always emitted unless they are suppressed.
