...
- **Google Antigravity (`antigravity`)** — Antigravity 2.0 splits into two products with separate global config trees: the desktop **`antigravity-ide`** and the **`antigravity-cli`** (`agy`). The legacy `antigravity` target is now a **deprecated alias for `antigravity-ide`** that keeps its original `.agent/` (singular) paths for backward compatibility. Migrate to `antigravity-ide` (desktop IDE) or `antigravity-cli` (CLI). The two split targets keep separate global skills trees (`~/.gemini/antigravity/skills/` vs `~/.gemini/antigravity-cli/skills/`). The `antigravity-cli` global MCP config now lives in the shared `~/.gemini/config/mcp_config.json` (matching the current Antigravity CLI docs), while the IDE still writes `~/.gemini/antigravity/mcp_config.json`. Both targets also intentionally **share** the global rule file `~/.gemini/GEMINI.md` and the global hooks file `~/.gemini/config/hooks.json` — enabling both targets in `--global` mode writes those shared files once.
